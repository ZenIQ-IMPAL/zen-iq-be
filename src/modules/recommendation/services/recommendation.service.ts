...
import { RecommendationFilters, RecommendedCourse } from '../types/recommendation.types';
...
export class RecommendationService {
  ...
  async getRecommendedCourses({ userId, lastNSearches = 5 }: RecommendationFilters): Promise<CourseWithInstructor[]> {
    ...
    const scoredCourses: RecommendedCourse[] = allCourses.map((course, i) => {
      ...
      const paddedQueryVector = [...queryVector, ...Array(maxLength - queryVector.length).fill(0)];
      const paddedCourseVector = [...courseVector, ...Array(maxLength - courseVector.length).fill(0)];
